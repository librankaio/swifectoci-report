...
class MutasiBhnBakuController extends Controller
{
    public function index(Request $request)
    {
        ...
        if (isset($request->dtfrom)) {
            if ($request->searchtext == null) {
                ...
                $results = DB::select('CALL LapMutasiBahanBakuOCI (?,?,?)', [$datefrForm, $datetoForm, $compcode]);

                // $query = DB::select('EXEC rptTest ?,?,?',[$datefrForm,$datetoForm,'BC 4.0']);

                $page = request('page', 1);
                $pageSize = 25;
                $query = DB::select('CALL LapMutasiBahanBakuOCI (?,?,?)', [$datefrForm, $datetoForm, $compcode]);
    ...
    public function exportExcel(Request $request)
    {
        ...
        $results = DB::select('CALL LapMutasiBahanBakuOCI (?,?,?)', [$datefrForm, $datetoForm, $comp_code]);
